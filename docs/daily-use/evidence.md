...
| D9 | — | — | — | — | not started |
| D10 | — | — | — | — | not started |
| D11 | `96d708f` — `test(d11): require object-to-data execution`; `8ecc0c7` — `test(d11): require selected object editor flow`; `8d297b1` — `test(d11): require result inspection modes`; `c259bbf` — `test(d11): require rendered workspace splitter` | `3b573a2` — `feat(d11): execute table data action`; `164346b` — `feat(d11): retain selected object context`; `740e483` — `feat(d11): add result inspection modes` | Object/editor slices: focused all-features contracts, `just check`, `just check-all`. Result inspection: focused `ui::result_view::tests::filter_and_sort_are_local_stable_and_debug_redacts_filter_text` and `result_grid_exposes_filter_sort_and_keyboard_record_detail`, then `just check` and `just check-all`. Splitter RED: `cargo test --all-features --lib ui::app::tests::actual_wide_splitter_is_accessible_keyboard_adjustable_and_resettable -- --exact --nocapture` fails because `workspace.splitter` is absent from the actual AX tree. | — | RED rendered-splitter slice; prior object/editor/result-inspection GREENs retained; complete D11 native journeys pending |
| D12 | — | — | — | — | not started |
