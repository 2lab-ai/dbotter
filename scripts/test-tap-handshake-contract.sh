...
formula="$tmp_dir/dbotter-preview.rb"
{
  ...
  printf '  def install\n'
  printf '    app = prefix/"Dbotter Preview.app"\n'
  printf '    if (buildpath/"Dbotter Preview.app").directory?\n'
  printf '      prefix.install "Dbotter Preview.app"\n'
  printf '    elsif (buildpath/"Contents").directory?\n'
  printf '      app.install "Contents"\n'
  printf '    else\n'
  printf '      odie "Dbotter Preview.app payload is missing"\n'
  printf '    end\n'
  printf '    bin.install_symlink app/"Contents/MacOS/dbotter" => "dbotter"\n'
  printf '  end\n\n'
  printf '  test do\n'
